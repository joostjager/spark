...
func (*TokenCreate) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case tokencreate.FieldIssuerPublicKey, tokencreate.FieldIssuerSignature, tokencreate.FieldOperatorSpecificIssuerSignature, tokencreate.FieldCreationEntityPublicKey, tokencreate.FieldMaxSupply:
			values[i] = new([]byte)
		case tokencreate.FieldIsFreezable:
			values[i] = new(sql.NullBool)
		case tokencreate.FieldWalletProvidedTimestamp, tokencreate.FieldDecimals:
	...
	for i := range columns {
		...
		case tokencreate.FieldDecimals:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				tc.Decimals = uint32(value.Int64)
			}
